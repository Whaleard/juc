...
线程等待、中断和阻塞
    线程等待：线程主动暂停执行，进入一个等待队列，知道被其他线程通过通知（如notify()或notifyAll()）唤醒或满足特定条件。
            在Java中，线程通过调用Object.wait()、Thread.join()或Lock的Condition.await()等方法进入等待状态。
    线程中断：一种协作式机制，用于通知一个线程它应该停止当前操作。调用线程的interrupt()方法会设置中断标志位，但线程本身不会立即终止；
            需要主动检查中断状态（如通过Thread.interrupted()或isInterrupted()）并做出响应。
    线程阻塞：线程被动暂停执行，直到外部条件满足（如获取锁、I/O操作完成）。阻塞通常由操作系统或JVM控制，线程会被挂起并释放CPU资源。
...
sleep()和wait()
    ...
    不同点：
        ...
        2、sleep()调用后不会释放持有的锁，调用前也不需要占用锁；wait()调用后会‌立即释放锁‌，但调用它的前提是当前线程占有锁（即synchronized块或方法内）。
...
公平锁与非公平锁：
    公平锁：遵循“先来先服务”的原则，线程按照请求锁的顺序依次获取锁。公平锁效率相对低，线程不会饿死
    非公平锁：默认的锁实现，线程尝试直接竞争锁，不关注排队顺序。非公平锁效率高，但容易造成线程饿死

可重入锁
    以线程为单位，当一个线程获取到对象锁之后，在未释放锁之前可以再次获取本对象上的锁，而不会产生死锁。

乐观锁和悲观锁
    乐观锁：认为共享资源每次被访问的时候不会出现问题，线程可以不停地执行，无需加锁也无需等待，只是在提交修改的时候去验证对应的资源是否被其它线程修改了。例如表里加版本字段每次修改前判断版本字段是否被修改
    悲观锁：认为共享资源每次被访问的时候就会出现问题，所以在每次获取资源操作的时候都会上锁

表级锁和行级锁
    定义：
        表级锁：锁定整张表，任何操作都会阻塞其他事务的写操作（甚至部分读操作）。
        行锁：仅锁定需要操作的具体行，允许其他事务访问未锁定行
    特点：
        表锁不会产生死锁，MySQL的表锁是一次性获取的。
        行锁会产生死锁
...
产生死锁原因
    1、互斥：资源一次只能被一个进程独占使用。
    2、持有并等待：进程在持有资源的同时等待被其他进程占用的资源。
    3、不可剥夺：已分配给进程的资源不能被强制抢占。
    4、循环等待：存在一个进程-资源的环形等待链，即每个进程都在等待下一个进程所占用的资源，例如A等待B的资源，B等待C的资源，C等待A的资源。

如何避免死锁
    1、破坏持有并等待条件：
        (1)一次性获取所有资源，例如MySQL的表锁就是一次性获取的
        (2)超时释放机制
    2、破坏不可剥夺条件：允许系统强制回收资源（如高优先级进程抢占），但需支持事务回滚
